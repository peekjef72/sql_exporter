...
func TargetsHandlerFunc(metricsPath string, exporter Exporter) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		type targets struct {
			Tgs []*dumpTargetConfig `yaml:"targets" json:"targets"`
		}
		var (
			tgs         *targets
			targets_cfg []byte
		...
		if ctxval.path != "" {
			if tg, err := exporter.FindTarget(ctxval.path); err == nil {
				tgl := make([]*dumpTargetConfig, 1)
				tgl[0] = tg.Config().buildDumpTargetconfig()
			...
			tgl := make([]*dumpTargetConfig, len(c.Targets))
			for i, t := range c.Targets {
				tgl[i] = t.buildDumpTargetconfig()
			}
		...
		if strings.Contains(accept_type, applicationJSON) {
			...
			targets_cfg, err = yaml.Marshal(tgs)
			if err != nil {
				// content = nil
				HandleError(0, err, metricsPath, exporter, w, r)
				return
